...
import EarningsBarChartPickDate from '../Charts/EarningsBarChartPickDate';
...
export default function OverviewDayView({ data, date }) {
    let dailyData = empty;
    if (data && typeof data === 'object') {
        let flat = data;
        if (data.staffDetails?.$values?.length) {
            flat = data.staffDetails.$values[0];
        }
        dailyData = {
            totalEarnings: flat.totalEarnings ?? 0,
            sessionCount: flat.sessionCount ?? 0,
            inProgressSessionCount: flat.inProgressSessionCount ?? 0,
            hourlyEarnings: flat.hourlyEarnings?.$values ?? [],
        };
    }
        ...
        <>
            ...
            <Paper elevation={0} sx={statPaper}>
                <Typography variant="subtitle1" sx={statTitle}>
                    Earnings by Hour {date ? `on ${date}` : ''}
                </Typography>
                <EarningsBarChartPickDate hourlyEarnings={dailyData.hourlyEarnings} />
            </Paper>
        </>
    );
}